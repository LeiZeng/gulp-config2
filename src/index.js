...
function loadTask (obj) {
  ...
  if (obj.modules[obj.taskName] === clean) {
    return gulp.task(obj.taskName, function (cb) {
      clean(gconf.getConf(obj.taskName, 'src') || gconf.getConf('dest'))(cb)
    })
  }
  ...
  return gulp.task(obj.taskName, function () {
    ...
    return merge.apply(gulp, conf.map(config => {
      ...
      var srcOption = [].concat(config && config.src || gconf.getConf('src'))
      // src options
      if (config.srcOption) {
      ...
      if (config && config.dest || gconf.getConf('dest')) {
        src = src.pipe(
          gulp.dest(
            config.dest || gconf.getConf('dest'),
            config.destOption
          )
        )
      }
...
function loadModule (moduleName, taskName) {
  ...
  taskList[taskName] = taskList[moduleName || taskName] || require(moduleName)
...
function loadFromFile (fileName, taskName) {
  ...
  return loadModule(relative, taskName || path.basename(fileName))
}
